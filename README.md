...
### MySQL 架构和历史
- MySQL 最重要的特性是它的存储引擎架构，这种架构的设计将查询处理（Query Processing）及其他系统任务（Server Task）与数据的存储/提取相分离。这种**处理和存储**分离的设计可以在使用时根据性能、特性，以及其他需求来选择数据存储的方式。
...
- 隔离级别：较低的隔离级别通常可以执行更高的并发，系统的开销也更低。MySQL 修改隔离级别命令：`SET SESSION TRANSACTION ISOLATION LEVEL ...`
  ...
  - **提交读**（READ COMMITTED）：大多数数据库系统的默认隔离级别（MySQL 不是），一个事务在提交之前，所做的任何修改都是对其他事务不可见的。**会存在不可重复读、幻读的问题**
